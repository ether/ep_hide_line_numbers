# Contributing to ep_hide_line_numbers

Thanks for helping improve `ep_hide_line_numbers` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_hide_line_numbers` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_hide_line_numbers from your local clone
pnpm run plugins i --path ../ep_hide_line_numbers

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_hide_line_numbers/` are picked up after a server restart.
...
* Etherpad version, ep_hide_line_numbers version, browser + OS
* Steps to reproduce
